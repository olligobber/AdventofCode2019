...
updateBot state input
    | paintNext state = BotState
        ...
        (pos state)
        (dir state)
        (S.insert (pos state) (allPainted state))
        False
    | otherwise = BotState
        (whites state)
...
startBot :: BotState
startBot = BotState S.empty (Position 0 0) U (S.empty) True

main :: IO ()
main = interact $ show . length . allPainted . runforever . makeState . fmap read . words . fmap (\x -> if x == ',' then ' ' else x)
