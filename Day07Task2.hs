...
runOnPara :: [Int] -> Int -> [Int] -> [Int]
runOnPara code para input = runforever $ makeState code $ para:input

runSequence :: [Int] -> (Int, Int, Int, Int, Int) -> Int
runSequence code (paraA, paraB, paraC, paraD, paraE) = last outputE
    where
        run x = runOnPara code x
        outputA = run paraA (0:outputE)
        outputB = run paraB outputA
        outputC = run paraC outputB
        outputD = run paraD outputC
        outputE = run paraE outputD

bestSequence :: [Int] -> Int
bestSequence code = maximum $ do
    [a,b,c,d,e] <- permutations [5..9]
    return $ runSequence code (a,b,c,d,e)

main :: IO ()
main = interact $ show . bestSequence . fmap read . words . fmap (\x -> if x == ',' then ' ' else x)
