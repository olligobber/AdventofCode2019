...
getOreAmount :: Ingredients -> Integer
getOreAmount ingredients = case M.toList (amounts ingredients) of
    [("ORE", x)] -> x
    [] -> 0
    _ -> error "Ingredients needed other than ore"

fuelRequires :: Recipes -> Integer -> Integer
fuelRequires recipes =
    getOreAmount .
    eliminateAllBut recipes .
    Ingredients .
    M.singleton "FUEL"

binarySearchMax :: (Integer -> Bool) -> (Integer,Integer) -> Integer
binarySearchMax p (bottom, top)
    | p top = top
    | bottom == top = bottom
    | bottom+1 == top = bottom
    | p middle = binarySearchMax p (middle, top)
    | otherwise = binarySearchMax p (bottom, middle)
    where
        middle = (bottom + top) `div` 2

findMaxFuel :: Integer -> Recipes -> Integer
findMaxFuel ore recipes = binarySearchMax
    ((<= ore) . fuelRequires recipes)
    (0,ore)

main :: IO ()
main = interact $ (++ "\n") . show . findMaxFuel (10^12) . readRecipes
